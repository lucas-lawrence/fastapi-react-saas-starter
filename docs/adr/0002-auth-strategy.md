...
- **Opaque refresh tokens** (random 32-byte string, 30 day expiry) — stored in `refresh_tokens` table, used only to obtain a new access token
...
- Users stay logged in for up to 30 days without re-entering credentials
